...
import detectron.modeling.webly_heads as webly_heads
    ...
    def _single_gpu_build_func(model):
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            if cfg.WEBLY.WEBLY_ON:
                head_loss_gradients['box'] = _add_webly_head(
                    model, add_roi_box_head_func, blob_conv, dim_conv,
                    spatial_scale_conv
                )
            else:
...
def _add_webly_head(
    ...
    webly_heads.add_webly_outputs(model, blob_frcn, dim_frcn)
    if model.train and cfg.WEBLY.MINING:
        webly_heads.add_webly_mining(model)

        blob_frcn, dim_frcn = get_func(cfg.FAST_RCNN.ROI_BOX_HEAD + '_shared')(
            model, blob_in, dim_in, spatial_scale_in, prefix='mining_'
        )

        webly_heads.add_webly_outputs_shared(model, blob_frcn, dim_frcn, prefix='mining_')
        loss_gradients = webly_heads.add_webly_losses(model, prefix='mining_')
    elif model.train:
        loss_gradients = webly_heads.add_webly_losses(model)
    else:
        loss_gradients = None
    return loss_gradients
