#ifndef SOFTMAX_WITH_LOSS_N_OP_H_
#define SOFTMAX_WITH_LOSS_N_OP_H_
...
class SoftmaxWithLossNOp final : public Operator<Context> {
 public:
  template <class... Args>
  explicit SoftmaxWithLossNOp(Args&&... args)
 ...
 protected:
  ...
  Tensor weights_valid_; // unignored weights
...
template <typename T, class Context>
class SoftmaxWithLossNGradientOp final : public Operator<Context> {
 public:
  template <class... Args>
  explicit SoftmaxWithLossNGradientOp(Args&&... args)
      : Operator<Context>(std::forward<Args>(args)...),
        scale_(this->template GetSingleArgument<float>("scale", 1.)),
        label_prob_mode_(
            this->template GetSingleArgument<int>("label_prob", 0)),
...
#endif // SOFTMAX_WITH_LOSS_N_OP_H_
