...
__global__ void ROIPoolForward(
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    int roi_start_w_in = roundf(offset_bottom_rois[5] * spatial_scale);
    int roi_start_h_in = roundf(offset_bottom_rois[6] * spatial_scale);
    int roi_end_w_in = roundf(offset_bottom_rois[7] * spatial_scale);
    int roi_end_h_in = roundf(offset_bottom_rois[8] * spatial_scale);
...
template <>
bool RoILoopPoolOp<float, CUDAContext>::RunOnDevice() {
...
template <>
bool RoILoopPoolGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& X = Input(0); // Input data to pool
  auto& R = Input(1); // RoIs
...
REGISTER_CUDA_OPERATOR(RoILoopPool, RoILoopPoolOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(RoILoopPoolGradient, RoILoopPoolGradientOp<float, CUDAContext>);
