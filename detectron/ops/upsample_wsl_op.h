...
class UpsampleBilinearWSLOp final : public Operator<Context> {
 public:
  UpsampleBilinearWSLOp(const OperatorDef& operator_def, Workspace* ws)
...
template <typename T, class Context>
class UpsampleBilinearWSLGradientOp final : public Operator<Context> {
 public:
  UpsampleBilinearWSLGradientOp(const OperatorDef& operator_def, Workspace* ws)
      : Operator<Context>(operator_def, ws), width_scale_(1), height_scale_(1) {
    width_scale_ = static_cast<T>(
        this->template GetSingleArgument<float>("width_scale", 1));
    height_scale_ = static_cast<T>(
