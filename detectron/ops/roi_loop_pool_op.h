#ifndef ROI_LOOP_POOL_OP_H_
#define ROI_LOOP_POOL_OP_H_
...
class RoILoopPoolOp final : public Operator<Context> {
 public:
  template <class... Args>
  explicit RoILoopPoolOp(Args&&... args)
...
template <typename T, class Context>
class RoILoopPoolGradientOp final : public Operator<Context> {
 public:
  template <class... Args>
  explicit RoILoopPoolGradientOp(Args&&... args)
      : Operator<Context>(std::forward<Args>(args)...),
        spatial_scale_(
            this->template GetSingleArgument<float>("spatial_scale", 1.)),
        pooled_height_(this->template GetSingleArgument<int>("pooled_h", 1)),
