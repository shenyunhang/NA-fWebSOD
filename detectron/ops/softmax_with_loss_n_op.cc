...
REGISTER_CPU_OPERATOR(SoftmaxWithLossN, SoftmaxWithLossNOp<float, CPUContext>);
REGISTER_CPU_OPERATOR(
    SoftmaxWithLossNGradient,
    SoftmaxWithLossNGradientOp<float, CPUContext>);

// Input: X (logits), T (labels); Output: P (probs), Y
OPERATOR_SCHEMA(SoftmaxWithLossN)
    .NumInputs(2, 3)
    .NumOutputs({2, 3})
...
OPERATOR_SCHEMA(SoftmaxWithLossNGradient).NumOutputs(1);

#define DONT_CARE (-1)

template <>
bool SoftmaxWithLossNOp<float, CPUContext>::RunOnDevice() {
...
template <>
bool SoftmaxWithLossNGradientOp<float, CPUContext>::RunOnDevice() {
  auto& X = Input(0); // Logits
  auto& T = Input(1); // Labels / targets
...
namespace {
class GetSoftmaxWithLossNGradient : public GradientMakerBase {
  ...
  vector<OperatorDef> GetGradientDefs() override {
    ...
    return SingleGradientDef(
        "SoftmaxWithLossNGradient", "", blob_names, vector<string>{GI(0)});
  }
};

REGISTER_GRADIENT(SoftmaxWithLossN, GetSoftmaxWithLossNGradient);
} // namespace
} // namespace caffe2
