...
import detectron.utils.feat_vis as feat_vis
...
def multi_gpu_test_net_on_dataset(
    ...
    binary = os.path.join(binary_dir, 'test_net_wsl_vis' + binary_ext)
...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        if cfg.TEST.PRECOMPUTED_PROPOSALS:
            ...
            # Uncomment for VIS only
            box_proposals = entry['boxes']
            obn_scores = entry['obn_scores']
...
def initialize_model_from_cfg(weights_file, gpu_id=0):
    ...
    # net_utils.initialize_gpu_from_weights_file(
        # model, 'models/VGG/VGG_ILSVRC_16_layers_v1.pkl', gpu_id=gpu_id,
    # )
    # net_utils.initialize_gpu_from_weights_file(
        # model, 'models/DRN-WSOD/resnet18y_3st_model_120.pkl', gpu_id=gpu_id,
    # )
    # net_utils.initialize_gpu_from_weights_file(
        # model, 'models/DRN_WSOD/resnet50ws_model_131.pkl', gpu_id=gpu_id,
    # )
    # net_utils.initialize_gpu_from_weights_file(
        # model, 'models/DRN-WSOD/resnet18_model_120.pkl', gpu_id=gpu_id,
    # )
    # net_utils.initialize_gpu_from_weights_file(
        # model, '/tmp/detectron-download-cache/ImageNetPretrained/MSRA/R-50.pkl', gpu_id=gpu_id,
    # )
    # net_utils.initialize_gpu_from_weights_file(
        # model, '/tmp/detectron-download-cache/ImageNetPretrained/MSRA/R-101.pkl', gpu_id=gpu_id,
    # )
