...
def draw2():
    ...
    fig = plt.figure()
    ax1 = fig.add_subplot(1, 1, 1)
    for i, log_path in enumerate(sys.argv[1:]):
        loss_values, iter_values, snapshot_value = get_loss(log_path)

        # scale epoch
        scale_epoch_values = [
            i * 30 / len(loss_values) for i in range(len(loss_values))
        ]

        log_path = sys.argv[1]
        output_dir = os.path.join('draw')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
    ...
    ax1.set_xlim([0, 30])
    ax1.set_ylim([0, loss_values[0]])
    plt.savefig(os.path.join(output_dir, 'all_loss.png'), dpi=1200)
