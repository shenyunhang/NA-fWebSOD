...
def normalize_resnet_name(name):
    name = name.replace('/', '_')
    if name.find('res') == 0 and name.find('res_') == -1 and False:
        # E.g.,
...
def remove_spatial_bn_layers_keep_bn(caffenet, caffenet_weights):
    ...
    for (bn, scl) in zip(bn_layers[0::2], bn_layers[1::2]):
        assert bn.name[len('bn'):] == scl.name[len('scale'):] or bn.name[:-len('bn')] == scl.name[:-len('scale')], 'Pair mismatch'
        blob_out = bn.name
...
def load_and_convert_caffe_model(prototxt_file_name, caffemodel_file_name):
    ...
    # bn_weights = remove_spatial_bn_layers_keep_bn(caffenet, caffenet_weights)
